...
class CredentialProvider:
    ...
    def __init__(self, account="default", credentials=None):
        ...
        self.read_credentials = [self.from_env, self.read_config]
        if credentials:
    ...
    def from_env(self):
        account_data = dict(
            refresh_token=self._get_env("MELI_API_REFRESH_TOKEN"),
            client_id=self._get_env("MELI_API_CLIENT_ID"),
            client_secret=self._get_env("MELI_API_CLIENT_SECRET"),
        )
