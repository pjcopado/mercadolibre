...
from meli.auth.credentials import Credentials
from meli.auth import AccessTokenClient, AccessTokenResponse
...
role_cache = TTLCache(
    maxsize=int(os.environ.get("AD_API_AUTH_CACHE_SIZE", 10)), ttl=3200
)
...
class Client(BaseClient):
    ...
    def _download(self, params: dict = None, headers=None) -> ApiResponse:
        ...
        try:
            ...
            error = {"success": False, "code": e.status_code, "response": e}
            next_token = None
            return ApiResponse(error, next_token, headers=self.headers)
        ...
        if mode == "raw":
            ...
            if bytes[0:2] == b"\x1f\x8b":
                ...
                next_token = None
                return ApiResponse(
                    json.loads(read_data.decode("utf-8")), next_token, headers=r.headers
                )
    ...
    def _check_response(self, res) -> ApiResponse:
        ...
        if (
            type(str_content) is str
            and str_content[0:50]
            == '<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN">'
            and vars(res).get("_content_consumed") is True
        ):
        ...
        if (
            type(str_content) is str
            and str_content[0:15] == "Invalid request"
            and vars(res).get("_content_consumed") is True
        ):
            dictionary = {
                "status_code": vars(res).get("status_code"),
        ...
        if (
            type(data) is dict
            and data.get("code") == "SERVER_IS_BUSY"
            and vars(res).get("_content_consumed") is True
        ):
            dictionary = {
                "status_code": vars(res).get("status_code"),
        ...
        if (
            type(data) is dict
            and data.get("message") == "Unauthorized"
            and vars(res).get("_content_consumed") is True
        ):
            dictionary = {
                "status_code": vars(res).get("status_code"),
        ...
        if (
            type(data) is dict
            and data.get("details") == "Invalid authorization inputs"
            and vars(res).get("_content_consumed") is True
        ):
            dictionary = {
                "status_code": vars(res).get("status_code"),
                "message": "Invalid authorization inputs",
            }
            exception = get_exception_for_content(dictionary)
            raise exception(dictionary)
        ...
        if (
            type(data) is dict
            and data.get("message") == "Missing Authentication Token"
            and vars(res).get("_content_consumed") is True
        ):
            dictionary = {
                "status_code": vars(res).get("status_code"),
        ...
        if (
            type(data) is dict
            and data.get("details") == "Cannot consume content type"
            and vars(res).get("_content_consumed") is True
        ):
            dictionary = {
                "status_code": vars(res).get("status_code"),
